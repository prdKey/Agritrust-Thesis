...
export default function ProductDetails({ product }) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await addToCart(product, quantity); // cartService expects (product, quantity)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col justify-between h-full">
          <div>
            ...
            <div className="flex items-center mt-2 gap-2 flex-wrap">
              ...
              <div className="flex items-center">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} className={`w-4 h-4 ${i < rating ? "text-yellow-400" : "text-gray-300"}`}
            ...
            {/* Price */}
            <p className="text-green-600 text-2xl font-bold mt-1">{product.pricePerUnit} AGT</p>
            ...
            <div className="flex items-center gap-3 mt-5">
              ...
              <div className="flex items-center justify-center border border-gray-300 rounded-lg overflow-hidden">
                ...
                >−</button>
                <span className="px-4 py-1 text-sm text-gray-700 font-medium min-w-[2rem] text-center">
                  {quantity}
                </span>
                <button
                  onClick={increment}
