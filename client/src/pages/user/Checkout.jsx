...
import {
  ...
  Plus, Minus, Trash2, AlertCircle, RefreshCw
} from "lucide-react";

// ── Axios for addresses/coupons ───────────────────────────────────────────────
...
export default function CheckoutPage() {
  ...
  const [orderSuccess, setOrderSuccess] = useState(null); // { orderIds, total }

  // ── Load items from CartPage state + fetch addresses ──────────────────────────
  useEffect(() => {
    if (!user) return;
  ...
  // ── Cart qty update (local only — CartPage already synced to backend) ─────────
  ...
  // ── Pricing (plain number math — pricePerUnit is a number from CartPage) ──────
  ...
  const discount = appliedCoupon
    ? appliedCoupon.type === "percent"
      ...
      : appliedCoupon.value
    : 0;
  // Logistics fee comes from contract (50 AGT per order), shown as info only
  const LOGISTICS_FEE_PER_ORDER = 50;
  const PLATFORM_FEE_RATE = 0.0005; // 0.05%
  const platformFee = subtotal * PLATFORM_FEE_RATE;
  const logisticsFee = cartItems.length * LOGISTICS_FEE_PER_ORDER;
  const total = Math.max(0, subtotal + platformFee + logisticsFee - discount);

  // ── Place order — calls orderService.buyProduct() per item ────────────────────
  const placeOrder = async () => {
    ...
    try {
      for (let i = 0; i < cartItems.length; i++) {
        ...
        // Uses your existing orderService → POST /orders/checkout
        const data = await buyProduct(item.productId, item.quantity);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-lg p-10 max-w-lg w-full text-center">
          ...
          <div className="flex gap-3">
            <button onClick={() => navigate("/orders")} className="flex-1 py-3 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition-colors">
              View Orders
            </button>
          ...
          <div className="lg:col-span-2 space-y-5">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-6 space-y-3">
                  {addresses.length === 0 ? (
                    ...
                    addresses.map(addr => (
                      <label key={addr.id} className={`flex gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedAddress === addr.id ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-gray-300"}`}>
                        <input type="radio" name="address" value={addr.id} checked={selectedAddress === addr.id} onChange={() => setSelectedAddress(addr.id)} className="accent-green-600 mt-0.5" />
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {addr.isDefault && <span className="text-xs bg-green-100 text-green-600 px-2 py-0.5 rounded-full font-medium">Default</span>}
                          </div>
                          <p className="text-sm text-gray-600">{addr.line1}{addr.line2 ? `, ${addr.line2}` : ""}</p>
                          <p className="text-sm text-gray-600">{addr.city}, {addr.province} {addr.zip}</p>
                          <p className="text-xs text-gray-400 mt-1">{addr.phone}</p>
                        </div>
                      </label>
                    ))
                  )}
                  <button onClick={() => navigate("/addresses/new")} className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-green-400 hover:text-green-600 transition-colors flex items-center justify-center gap-2">
                    <Plus className="w-4 h-4" /> Add New Address
                  </button>
                </div>
                <div className="px-6 pb-6">
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Order Note (optional)</label>
                  <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Special instructions, gate code, etc." rows={3} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-green-500" />
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    {selectedAddr ? (
                      <>
                        ...
                        <p className="text-sm text-gray-600">{selectedAddr.line1}</p>
                        <p className="text-sm text-gray-600">{selectedAddr.city}, {selectedAddr.province}</p>
                        <p className="text-xs text-gray-400 mt-1">{selectedAddr.phone}</p>
                      </>
                    ) : (
                      <p className="text-sm text-gray-400 italic">No address selected</p>
                    )}
                    <p className="text-xs text-green-600 font-medium mt-2">via {deliveryOpt?.label}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2 flex items-center gap-1">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              {appliedCoupon ? (
                <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-4 py-2.5">
                  ...
                  <button onClick={() => { setAppliedCoupon(null); setCouponCode(""); }} className="text-red-400 hover:text-red-600 transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
